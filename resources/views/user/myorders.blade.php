...
<link rel="stylesheet" href="{{url('admin_assets/css/no_record.css')}}">
<div class="title mx-4" style="margin-top:100px;">
        <h1>My Orders</h1>
</div>
<div class="my-4 pb-3 bg-light" style="margin-left:20px; margin-right:20px; border-radius:15px;">
    <table class="table table-hover">
        <thead class="text-light" style="background-color: rgba(254, 72, 5, 0.86);">
            <tr>
        ...
        <tbody>
            ...
            <tr>
                <td>#OFS0{{$ProductData->id}}</td>
                <td>
                    <img src="{{url('/admin_uploads/product_images/')}}/{{$ProductData->product_image}}" alt="" style="width:100px;height:80px;border-radius:10px;">
                </td>
                <td>
                    <p>
                        <span class='text-capitalize' style="font-family: sans-serif;color:#72A0CD;">Product Name :{{$ProductData->product_name}}</span><br>
                        Price : Rs.{{$ProductData->product_price}}<br>
                        Qty : {{$ProductData->qty}}<br>
                        Subtotal : Rs.{{$ProductData->product_price*$ProductData->qty}}<br>
                        
                    </p>
                </td>
                <td>
                    <p>
                        <span class='text-capitalize' style="font-family: sans-serif;color:#72A0CD;">  Recipient Name : {{$ProductData->name}}</span><br>
                ...
                <td>
                        <a href="{{url('user/homepage/myorders/check_status/')}}/{{$ProductData->id}}/{{$ProductData->product_id}}">
                            <button class="btn btn-success btn-sm">Check Status</button>
                        </a>
...
@else
<div class="empty-state text-center container w-50 border-1" style="margin-top:200px;">
  <div class="empty-state__content">
    ...
    <div class="empty-state__message">No orders has been added yet.</div>
    <div class="empty-state__help">
      <a href="{{route('homepage')}}" class="btn btn-success my-2">Order Now</a>
    </div>
  </div>
</div>
@endif
